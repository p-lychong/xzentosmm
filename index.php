...
$panel_orders = $settings["panel_order"];
...
$osp_orders = $panel["panel_limit_order"]-$panel["panel_orders"];
  ...
  if( route(0) != "admin" &&  route(0) != "osp_rental" && $settings["site_maintenance"] == 1 ): include 'app/views/maintenance.php';exit(); endif;
 ...
 $unpaidCount =  countRow(["table"=>"invoices","where"=>["invoice_status"=>"Unpaid" , "client_id" => $user["client_id"]]]);
	...
	if( $_SESSION["msmbilisim_userlogin"] != 1  || $user["client_type"] == 1  ){
      ...
      array(
            ...
            'styleList'=>$stylesheet["stylesheets"],'scriptList'=>$stylesheet["scripts"],'captchaKey'=>$settings["recaptcha_key"],'captcha'=>$captcha,'resetPage'=>$resetPage,'serviceCategory'=>$categories,'categories'=>$categories,'category_icon'=>$categories['category_icon'],
            ...
            'ticketCount'=>new_ticket($user['client_id']),'paymentsList'=>$methodList,'integrationsList'=>$methoList,'transactions'=>$transaction_logs,'chilpanel_logs'=>$panel_logs,'decoration'=>$decoration,'ospphonepe'=>$phonepe["method_type"],'PaytmQR'=>$PaytmQR["method_type"],'PaytmQRimage'=>$PaytmQRimage,'WhatsApp'=>$WhatsApp["method_type"],'whatsappstatus'=>$whatsappstatus,'whatsappvisibility'=>$whatsappvisibility,'bankPayment'=>$bankPayment["method_type"],'bankList'=>$bankList,
            ...
            'footerCode'=>$settings["custom_footer"],'lang'=>$languageArray,'timezones'=>$timezones,'AllOrders'=>$panel_orders,'whatsappbutton'=>$settings["whatsappbutton"],'whatsappnumber'=>$settings["whatsappnumber"],'whatsappcolour'=>$settings["whatsappcolour"],'whatsappposition'=>$settings["whatsappposition"],'telegrambutton'=>$settings["telegrambutton"],'telegramusername'=>$settings["telegramusername"],'telegramcolour'=>$settings["telegramcolour"],'telegramposition'=>$settings["telegramposition"],'osp_img'=>$settings["theme_bg_img"], 'panel'=>$panel, 'general'=>$general, 'updates'=>$updates , 'blogs'=>$blogs, 'blogtitle'=>$blogtitle, 'blogcontent'=>$blogcontent, 'blogimage'=>$blogimage, 'currency'=>$currency, 'currencies'=>$currencies, 'menus'=>$menus, 'MenuSlug'=>$menuslug, 'pageurl'=>$pageurl , 'pagekeywords'=>$seo_keywords, 'pagetitle'=>$seo_title, 'pagedescription'=>$seo_description, 'PageText'=>$PageText
      )
    );
 ...
 $unpaidCount =  countRow(["table"=>"invoices","where"=>["invoice_status"=>"Unpaid" , "client_id" => $user["client_id"]]]);
	 echo $twig->render( $templateDir.'.twig',
      array(
            ...
            'styleList'=>$stylesheet["stylesheets"],'scriptList'=>$stylesheet["scripts"],'captchaKey'=>$settings["recaptcha_key"],'captcha'=>$captcha,'resetPage'=>$resetPage,'serviceCategory'=>$categories,'decoration'=>$decoration,'ospdecoration'=>$ospdecoration,'categories'=>$categories,'category_icon'=>$categories['category_icon'],
            'error'=>$error,'errorText'=>$errorText,'success'=>$success,"servicesPage"=>$serviceList,"resetType"=>$resetType,'successText'=>$successText,'title'=>$title,"Signup"=>$Signup,"ChildPanel"=>$ChildPanel,'affiliatessystem'=>$affiliatessystem,'Promotion'=>$Promotion,
